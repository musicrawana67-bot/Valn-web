...
postBtn.addEventListener('click', () => {
  ...
  if (!caption && !currentPhoto) { pulse(postBtn, '#ff2244'); return; }
  ...
  card.innerHTML = `
    ${currentPhoto ? `<img src="${currentPhoto}" alt="post"/>` : `<div class="no-img">◈</div>`}
    <div class="card-body">
...
/* ══════════════════════════════════════
   BACKGROUND MUSIC  (217KB — auto)
   දෙවැනියට දාපු track — page load වෙනවිටම
   ══════════════════════════════════════ */
...
function playBg() {
  bgMusic.play().then(() => {
    ...
    events.forEach(ev => document.removeEventListener(ev, playBg));
  }).catch(() => {});
}

const events = ['click','touchstart','keydown','scroll','mousemove'];
events.forEach(ev => document.addEventListener(ev, playBg, { passive: true }));
